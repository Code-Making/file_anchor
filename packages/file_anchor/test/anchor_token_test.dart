  ...
  group('AnchorToken', () {
    test('round-trips every kind', () {
      for (final kind in AnchorKind.values) {
        const payload =
            'content://com.android.externalstorage/tree/primary%3ADocs';
    ...
    test('survives payloads with dots, slashes and unicode', () {
      ...
      final decoded = AnchorToken.parse(
        AnchorToken.of(AnchorKind.path, payload).value,
      );
      expect(decoded.payload, payload);
    });
    ...
    test('rejects a foreign string', () {
      expect(
        () => AnchorToken.parse('/Users/me/Documents'),
    ...
    test('equality is by value, so tokens work as map keys', () {
      expect(
        AnchorToken.of(AnchorKind.saf, 'a'),
        AnchorToken.of(AnchorKind.saf, 'a'),
      );
      expect(
        AnchorToken.of(AnchorKind.saf, 'a'),
        isNot(AnchorToken.of(AnchorKind.path, 'a')),
      );
    });
  });
}
